...
export default async function ProfileDetailPage({
  ...
  // Fetch profile + genome jobs in parallel
  const [profiles, jobs] = await Promise.allSettled([
    getProfiles(),
    getGenomeJobs(id),
  ])
  ...
  const profile =
    profiles.status === "fulfilled"
      ? profiles.value.find((p: any) => p.id === id)
      : null

  if (!profile) notFound()

  const genomeJobs = jobs.status === "fulfilled" ? jobs.value : []
  const hasProcessedGenome = genomeJobs.some(
    (j) => j.status === "file_processed"
  )

  const birthDateStr = [
    profile.birth_year,
    profile.birth_month
      ? new Date(0, profile.birth_month - 1).toLocaleString("en", {
          month: "long",
        })
      : null,
    profile.birth_day,
    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <SectionCard title="Profile Details">
            ...
            <InfoRow
              icon={Calendar}
              label="Date of Birth"
              value={birthDateStr || profile.birth_year}
            />
            {profile.height && (
              <InfoRow
                icon={Ruler}
            ...
            {profile.secondary_ethnicity && (
              <InfoRow
                icon={Globe}
                label="Secondary Ethnicity"
                value={profile.secondary_ethnicity}
              />
            )}
          </SectionCard>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <SectionCard
            title="Upload Genome File"
            description="Supports 23andMe, AncestryDNA, and VCF formats"
          >
